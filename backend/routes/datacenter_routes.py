...
def create_data_center(dc: DataCenterCreate, db: Session = Depends(get_dc_db)):
...
def get_data_centers(db: Session = Depends(get_dc_db)):
...
def delete_data_center(dc_id: int, db: Session = Depends(get_dc_db)):
